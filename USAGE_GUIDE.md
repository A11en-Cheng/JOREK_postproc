...
除了通过 Python 代码调用，工具包还提供了开箱即用的命令行脚本，能够快速对输出数据执行完整的重整化和出图流程。

**基本语法**:
```bash
python -m jorek_postproc.cli -f <文件路径> -t <时间步> -n <变量名> [可选参数]
```

**命令行参数**:
- `-f, --file`: (必选) JOREK边界量 `.dat` 数据文件路径
- `-t, --timesteps`: (必选) 空格分隔的时间步列表（如 4200 4650 5000）
- `-n, --name`: (必选) 要处理的物理量名称（对应文件中特定列，如 `heatF_tot_cd`，热通量）
- `-d, --device`: (可选) 选择装置掩膜，默认 `EXL50U`
- `--log_norm`: (可选) 是否对绘图启用对数分布着色（处理相差悬殊的数值级必备）
- `--limits`: (可选) 限定物理量上下限（如 `--limits 1e5 3e8`）
- `-o, --output`: (可选) 图片导出目录
- `--debug`: (可选) 开启详细调试打印信息 
