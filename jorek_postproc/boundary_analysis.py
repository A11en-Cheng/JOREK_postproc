...
def run_boundary_analysis(conf: cfg.ProcessingConfig):
    ...
    for ts in conf.timesteps:
        ...
        if np.sum(~non_zero_mask) > 0:
            if conf.debug:
                print(f"  Dropped {np.sum(~non_zero_mask)} zero rows")
        ...
        if device_geo and device_geo.masks:
            ...
            region_style = {
                ...
                "mask_LO": {
                    "label": "OL",
                    "color": "orange",
                },  # Changed OL to purple to be distinct from IL blue
            }
