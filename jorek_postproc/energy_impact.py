...
def compute_delta_q_batch(t_raw, q_raw_batch, t_eval, n_points=INTEGRATION_POINTS):
    ...
    interp_func = interp1d(
        ...
        axis=-1,
        bounds_error=False,
        fill_value=(q_raw_batch[:, 0], q_raw_batch[:, -1]),
        assume_sorted=True,
    )
...
def load_timestep_data(args):
    ...
    for file_path in candidates:
        if debug:
            print(f"Checking file for step {ts}: {file_path}")

        try:
            # 使用包内的读取函数
            col_names, blocks, time_mapping = read_boundary_file(
            ...
            # 找到了 block
            if debug:
                print(f"  [Worker] Found step {ts} in {file_path}, block: {block_key}")
            ...
            if np.sum(~non_zero_mask) > 0:
                if debug:
                    print(f"  [Worker] 丢弃 {np.sum(~non_zero_mask)} 行全零数据")
            ...
            grid_data = reshape_to_grid(block_data, col_names, names, xpoints=xpoints, debug=debug)

            return time_now, grid_data

        except Exception as e:
            if debug:
                print(f"  [Worker] 读取/处理文件 {file_path} 出错: {e}")
            continue

    if debug:
        print(f"  [Worker] 未找到时间步 {ts} 的数据 (检查了 {len(candidates)} 个文件)")
    return time_now, None
...
def make_test_dataset():
    ...
    data_set = {}
    data_set[0.0e0] = np.zeros((300, 300), dtype=np.float32)  # 添加初始时间步
    ...
    for j in range(0, 3):
        for i in range(0, 3):
            data_set[normalize_factor][100 * i : 100 * (i + 1), 100 * j : 100 * (j + 1)] = (
                np.float32(10 ** (i + j + 1))
            )
    data_set[2.0e0 * normalize_factor] = np.zeros((300, 300), dtype=np.float32)
    data_set[2.0e3 * normalize_factor] = np.zeros((300, 300), dtype=np.float32)
...
def run_energy_impact_analysis(conf):
    ...
    print(
        f"[EnergyImpact] 并行加载 {len(conf.timesteps)} 个时间步的数据...\n 时间步：{conf.timesteps}"
    )
    ...
    DQ_flat = np.memmap(
        dq_memmap_path, dtype="float32", mode="w+", shape=(n_total_pixels, len(t_eval))
    )
    ...
    try:
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in tqdm(
                as_completed(futures), total=total_chunks, desc="Calculating Convolution"
            ):
    ...
    if ref_grid_data is not None:
        ...
        try:
            device = get_device_geometry(
                conf.device, ref_grid_data.R, ref_grid_data.Z, xpoints=xpoints, debug=conf.debug
            )
        ...
        for ts_val in tqdm(sorted_timesteps, desc="Plotting 3D"):
                ...
                if device.masks:
                    ...
                    for mname, mmask in device.masks.items():
                        if mname in region_style:
                            ...
                            all_regions.append(
                                {"label": st["label"], "mask": mmask, "color": st["color"]}
                            )
            ...
            for mask_name, mask in device.masks.items():
                ...
                if conf.dim == "2d":
                    # 2D 展开图 (部件高亮)
                    fname = (
                        f"energy_impact_2d_{t_phys*1.e3:.5f}ms_ts{int(idx)}_{mask_name}{suffix}.png"
                    )
                    save_path = os.path.join(output_dir, fname)

                    current_regions = [
                        {"label": mask_name.replace("mask_", ""), "mask": mask, "color": "red"}
                    ]
