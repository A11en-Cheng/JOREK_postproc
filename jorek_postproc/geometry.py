...
class BaseDevice(ABC):
    ...
    def get_masks_and_angles(
        self, R: np.ndarray, Z: np.ndarray, debug: bool = False
...
class EXL50UDevice(BaseDevice):
    ...
    default_xpoints = np.array([[0.72, 0.877], [0.72, -0.877]])
...
def get_device_instance(device_name: str) -> BaseDevice:
    ...
    if device_name_upper in _DEVICE_REGISTRY:
        ...
        raise ValueError(
            f"Unknown device: {device_name}. Supported: {list(_DEVICE_REGISTRY.keys())}"
        )
...
def get_device_geometry(
    ...
    return DeviceGeometry(name=device.name, masks=masks, view_angles=angles, xpoints=final_xpoints)


def register_custom_device(device_name: str, device_class: Type[BaseDevice]) -> None:
    """
