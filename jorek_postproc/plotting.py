...
def plot_scatter_3d(
    ...
    # 绘制散点
    sc = ax.scatter(R, phi, Z, c=val, cmap=config.cmap, s=2, alpha=0.5, norm=norm)
    ...
    ax.set_xlabel(rf"$R$ Axis", fontsize=16)
    ax.set_ylabel(rf"$\phi$ Axis", fontsize=16)
    ax.set_zlabel(rf"$Z$ Axis", fontsize=16)
...
def plot_surface_3d(
    ...
    if not data.is_2d_grid():
        raise ValueError(
            "Data must be in 2D grid format for surface plotting. "
    ...
    cbar = fig.colorbar(sm, ax=ax, pad=0.1)
    # cbar.set_label(data.data_name, rotation=270, labelpad=15)
    ax.set_xlabel(rf"$R$ Axis", fontsize=10)
    ax.set_ylabel(rf"$\phi$ Axis", fontsize=10)
    ax.set_zlabel(rf"$Z$ Axis", fontsize=10)
    ax.view_init(elev=view_angle[0], azim=view_angle[1])
...
def plot_max_point(R, Z, phi, val, ax):
    ...
    ax.scatter([plot_R], [max_phi], [plot_Z], color="red", s=40, label=f"Max: {max_value:.2e}")
    # ax.legend()
...
def plot_heat_flux_analysis(
    ...
    if data.theta is None:
        if debug:
            print("[Plotting] Warning: No theta data available, cannot plot phi-theta map.")
        return

    if debug:
        print("[Plotting] Generating heat flux analysis plots...")
        print(
            f"  Data shape: R{data.R.shape}, Z{data.Z.shape}, phi{data.phi.shape}, theta{data.theta.shape}, data{data.data.shape}"
        )
    ...
    if config.show_left_plot and config.show_right_plot:
        ...
        if debug:
            print("[Plotting] No plots selected, skipping.")
        return
    ...
    if ax1 is not None:
        try:
            if data.is_2d_grid():
                ...
                if debug:
                    print("Data is not grid, skipping R-Z plot")
                if ax2 is None:
                    return  # nothing to plot

            ax1.plot(
                R_pol, Z_pol, "gray", linewidth=1.5, alpha=0.9, label="Boundary"
            )  # Changed to gray
    ...
    if ax2 is not None:
        ...
        if use_len:
            if debug:
                print("[Plotting] Using Arc Length for Y-axis")
            y_axis_raw = data.arc_length[0, :]
            y_label = r"Length [m]"
        else:
            if debug:
                print("[Plotting] Using Theta for Y-axis")
        ...
        mesh = ax2.pcolormesh(
            phi_axis, y_axis_sorted, val.T, cmap="viridis", norm=norm, shading="gouraud"
        )
            ...
            for reg in regions:
                if "mask" in reg:
                    ...
                    if np.any(mask_pol):
                        ...
                        ax2.text(
                            line_x + (phi_axis.max() - phi_axis.min()) * 0.01,
                            (y_min + y_max) / 2,
                            f"{reg['label']}",
                            color=reg["color"],
                            va="center",
    ...
    if save_path:
        ...
        if debug:
            print(f"Saved heat flux analysis to {save_path}")
    else:
        plt.show()
