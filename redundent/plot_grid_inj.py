...
def plot_mesh_system_point(grid_file=None, arrow=[], title=None,output_name=None):
    """
    绘制网格系统
    ...
    for arr in arrow:
        p1 = (arr[0], arr[1])
        p2 = (arr[2], arr[3])
        ax.scatter([p1[0]], [p1[1]], color="#00B928", s=100, zorder=5)  # 起点标记
        ax.scatter([p2[0]], [p2[1]], color="#00B928", s=100, zorder=5)  # 终点标记
        ax.annotate('', xy=p2, xytext=p1,zorder=6,
             arrowprops=dict(color="#FF3B3B", shrink=0, width=1.5, headwidth=5, headlength=7))
...
if __name__ == "__main__":
    ...
    # 2. 绘图
    # 将你的文件名填入这里
    plot_mesh_system_point(
        grid_file="/home/ac_desktop/XL50-U/XL50-U_1.4.5/grid_xpoint.dat", 
        arrow=[(0.9, 0.7,0.7, 0.6)],
        title="IMP_injection Grid"
    )