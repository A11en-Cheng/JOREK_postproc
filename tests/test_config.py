...
class TestProcessingConfig:
    ...
    def test_default_init(self):
        ...
        assert (
            conf.input_file == "test.dat"
            if hasattr(conf, "input_file")
            else conf.file_path == "test.dat"
        )
    ...
    def test_post_init_2d(self):
        ...
        conf = ProcessingConfig(
            file_path="test.dat", timesteps=["100"], dim="2d", plot_surface=True
        )
    ...
    def test_post_init_energy_impact_flag(self):
        ...
        conf = ProcessingConfig(file_path="test.dat", timesteps=["100"], energy_impact=True)
        assert conf.mode == "energy_impact"

    def test_post_init_plot_set_defaults(self):
        """测试plot_set模式默认维度"""
        conf = ProcessingConfig(file_path="test.dat", timesteps=["100"], mode="plot_set")
        assert conf.dim == "2d"
